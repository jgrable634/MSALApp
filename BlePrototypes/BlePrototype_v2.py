            ...
            async with bleak.BleakClient(device) as client:
                ...
                for service in services:
                    ...
                    if service.description == "Unknown":
                        ...
                        # Services just have the name as the key, uuid as a data point, and a inner map of a services' characteristics as a second data point.
                        if user_input != 'n':
                    ...
                    for characteristic in characteristics:
                        ...
                        if service.description == "Unknown":
                            ...
                            if user_input != 'n':
                                service_characteristic_map[service_description]['characteristics'][user_input] = {'uuid': characteristic.uuid, "can_read" : True if 'read' in characteristic.properties else False}
                                service_description = user_input
                            else: 
                                service_characteristic_map[service_description]['characteristics']["Unknown" + str(unknownCharacteristic)] = {'uuid': characteristic.uuid, "can_read" : True if 'read' in characteristic.properties else False}    
                                unknownCharacteristic = unknownCharacteristic+1
                        else:
                            service_characteristic_map[service_description]['characteristics'][characteristic.description] = {'uuid': characteristic.uuid, "can_read" : True if 'read' in characteristic.properties else False}
...
# Allows user to mark the characteristic for monitoring
# Parameters    
    # map: A hashmap of all services and characteristics' names and UUIDs established in Stage 0
...
async def view_characteristic(map, client, service, characteristic):
        ...
        if added is True:
            expected_responses = ['r', 'b']
            user_input = get_input("You can remove (r) this characteristic from the Marked Characteristics or you can go back (b):", expected_responses, None)
            if user_input == 'r':
            ...
            user_input = get_input("You can add (a) this characteristic from the Marked Characteristics or you can go back (b):", expected_responses, None)
            if user_input == 'a':
                # Checks to see if it can be read, otherwise program will not add characteristic
